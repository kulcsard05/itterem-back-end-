    ...
    namespace BackEndAlap.Services 
    {
        ...
        public class RendelesekController : ControllerBase
        {
            ...
            public async Task GetRendelesStream(CancellationToken ct)
            {
                ...
                // Első lekérés csatlakozáskor
                await GetRendelesek(Response, ct);
            ...
            [HttpGet]
            public async Task<IActionResult> GetRendelesek(HttpResponse response, CancellationToken ct)
            {
                using (var cx = new BackEndAlapContext())
                {
                    ...
                    // SSE formátum: "data: JSON_SZÖVEG\n\n"
                    var json = System.Text.Json.JsonSerializer.Serialize(responseData);
                    await response.WriteAsync($"data: {json}\n\n", ct);

                    // Ez kényszeríti ki, hogy az adat azonnal elmenjen a hálózaton
                ...
                try
                {
                    using (var cx = new BackEndAlapContext())
                    {
                        ...
                        else
                        {
                            ...
                            // Add the new order to the context and save to generate ID
                            cx.Rendeleseks.Add(targetOrder);
                            await cx.SaveChangesAsync(); // Save to get the ID
                        }
                        ...
                        foreach (var item in orderDto.Items)
                        {
                            ...
                            {
                                RendelesId = targetOrder.Id, // Set foreign key
